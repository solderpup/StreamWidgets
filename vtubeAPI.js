...
vtubeAPI.connect = () => {
  ...
  vtubeAPI.websocket.addEventListener("error", (err) => {
    ...
    console.log(
      `Attempting reconnect to Vtube in ${
        settings.vtubeInterval / 1000
      } seconds`
    );
  ...
  vtubeAPI.websocket.on("open", async () => {
    ...
    fs.readFile("./vtube_token.txt", (err, token) => {
      if (err) {
        console.log(
          "Could not open Vtube Studio token file. Generating new token..."
        );
        getNewAuthToken();
      } else {
...
const getNewAuthToken = () => {
  ...
  vtubeAPI.sendRequest(authRequest).then((response) => {
    if (Object.hasOwn(response.data, "authenticationToken")) {
      ...
      console.log(
        `Attempting reconnect to Vtube Socket in ${
          settings.vtubeInterval / 1000
        } second(s)`
      );
      setTimeout(vtubeAPI.connect, settings.vtubeInterval);
...
const authenticate = (token) => {
  ...
  vtubeAPI.sendRequest(authRequest).then((response) => {
    if (!response.data.authenticated) {
      console.log(
        "Existing Vtube API token rejected, generating new auth token"
      );
      getNewAuthToken();
    } else {
      vtubeAPI.loggedIn = true;
...
vtubeAPI.parseSuperchat = (superChat) => {
  for (command of vtubeAPI.commands) {
    if (
      superChat.snippet.superChatDetails.tier >= command.tier &&
      superChat.snippet.displayMessage.includes(`!${command.triggerWord}`)
    ) {
      let request = {
        apiName: "VTubeStudioPublicAPI",
        apiVersion: "1.0",
